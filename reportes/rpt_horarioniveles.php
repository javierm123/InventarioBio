...
while ($reg=$rspta->fetch_object()) {
	...
	$ant=$pdf->GetY(); 		    
    for($j=0;$j<15; $j++){
			$pdf->MultiCell(13,9.5,utf8_decode($data[$j][0]),1,'L',1);
			$pdf->Sety($ant+$i);
			$pdf->SetX(23);
			if($data[$j][1]=='')
				$pdf->MultiCell(42,9.5,'',1,'L',0);
			else
				$pdf->MultiCell(42,3,utf8_decode(str_pad($data[$j][1],160,' ')),1,'L',0);
			$pdf->Sety($ant+$i);
			$pdf->SetX(65);
			if($data[$j][2]=='')
				$pdf->MultiCell(42,9.5,'',1,'L',0);
			else
				$pdf->MultiCell(42,3,utf8_decode(str_pad($data[$j][2],160,' ')),1,'L',0);
			$pdf->Sety($ant+$i);
			$pdf->SetX(107);
			if($data[$j][3]=='')
				$pdf->MultiCell(42,9.5,'',1,'L',0);
			else
				$pdf->MultiCell(42,3,utf8_decode(str_pad($data[$j][3],160,' ')),1,'L',0);
			$pdf->Sety($ant+$i);
			$pdf->SetX(149);
			if($data[$j][4]=='')
				$pdf->MultiCell(42,9.5,'',1,'L',0);
			else
				$pdf->MultiCell(42,3,utf8_decode(str_pad($data[$j][4],160,' ')),1,'L',0);
			$pdf->Sety($ant+$i);
			$pdf->SetX(191);
			if($data[$j][5]=='')
				$pdf->MultiCell(42,9.5,'',1,'L',0);
			else
				$pdf->MultiCell(42,3,utf8_decode(str_pad($data[$j][5],160,' ')),1,'L',0);
			...
			if($data[$j][6]=='')
				...
				$pdf->MultiCell(42,3,utf8_decode(str_pad($data[$j][6],160,' ')),1,'L',0);
